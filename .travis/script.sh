...
then
	docker run -t -v "${PWD}:/libfshfs" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfshfs && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfshfs;
