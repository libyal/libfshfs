...
int mount_file_entry_initialize(
     ...
     size_t name_length,
     libfshfs_file_entry_t *fshfs_file_entry,
     libcerror_error_t **error )
{
	...
	*file_entry = memory_allocate_structure(
	...
	( *file_entry )->fshfs_file_entry = fshfs_file_entry;
     ...
     libcerror_error_t **error )
{
	...
	if( hfs_time != 0 )
	{
		/* Convert the HFS timestamp into a FILETIME timestamp
		 */
	...
	*creation_time = filetime;
	...
	if( hfs_time != 0 )
	{
		/* Convert the HFS timestamp into a POSIX nanoseconds timestamp
		 */
		posix_time = (uint64_t) ( (int64_t) hfs_time - 2082844800 );
	}
	*creation_time = posix_time;
#endif
	return( 1 );
}
...
int mount_file_entry_get_sub_file_entry_by_index(
     ...
     libcerror_error_t **error )
{
	libfshfs_file_entry_t *sub_fshfs_file_entry = NULL;
	system_character_t *filename                  = NULL;
