...
#endif
{
	const char *description = \
		"Use fshfsmount to mount a Hierarchical File System (HFS) volume.";

	fshfstools_option_t options[ ] = {
		{ 'h', NULL, "shows this help" },
		{ 'o', "offset", "specify the volume offset in bytes" },
		{ 'v', NULL, "verbose output to stderr, while fshfsmount will remain running in the foreground" },
		{ 'V', NULL, "print version" },
#if defined( HAVE_LIBFUSE ) || defined( HAVE_LIBFUSE3 ) || defined( HAVE_LIBOSXFUSE )
		{ 'X', "extended_options", "extended options to pass to sub system" },
#endif
		{ 0, "volume", "a Hierarchical File System (HFS) volume" },
	...
	int number_of_options                       = (int) ( sizeof( options ) / sizeof( fshfstools_option_t ) );
	int verbose                                 = 0;
	...
	struct fuse_args fshfsmount_fuse_arguments  = FUSE_ARGS_INIT(1, fuse_argv);
#else
	...
	if( fshfstools_getopt_get_options_string(
	     options,
	     number_of_options,
	     options_string,
	     32 ) != 1 )
	                   ...
	                   options_string ) ) != (system_integer_t) -1 )
	{
		switch( option )
		{
			...
			default:
				...
				fshfstools_getopt_usage_fprint(
				 stdout,
				 program,
				 description,
				 options,
	...
	fshfsmount_dokan_options.SingleThread = TRUE;
#else
