...
int mount_fuse_getxattr(
     ...
     char *value,
     size_t size,
     uint32_t position FSHFSTOOLS_ATTRIBUTE_UNUSED )
#else
int mount_fuse_getxattr(
     const char *path,
     const char *name,
...
#endif
{
...
#if defined( __APPLE__ )
	FSHFSTOOLS_UNREFERENCED_PARAMETER( position )
#endif
	...
	{
		if( libfshfs_extended_attribute_get_size(
		     extended_attribute,
		     &value_data_size,
		     &error ) != 1 )
		{
     ...
     size_t size )
{
	     ...
	     extended_attribute_index++ )
	{
		...
		if( libfshfs_extended_attribute_get_utf8_name_size(
		     extended_attribute,
		...
		if( size > 0 )
		{
			...
			if( libfshfs_extended_attribute_get_utf8_name(
			     extended_attribute,
...
void mount_fuse_destroy(
      void *private_data FSHFSTOOLS_ATTRIBUTE_UNUSED )
{
	libcerror_error_t *error = NULL;
	static char *function    = "mount_fuse_destroy";

	FSHFSTOOLS_UNREFERENCED_PARAMETER( private_data )
