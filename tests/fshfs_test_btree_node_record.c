/*
 * Library btree_node_record type test program
 *
...
#include "../libfshfs/libfshfs_btree_node_record.h"

#if defined( __GNUC__ ) && !defined( LIBFSHFS_DLL_IMPORT )

/* Tests the libfshfs_btree_node_record_initialize function
 * Returns 1 if successful or 0 if not
 */
int fshfs_test_btree_node_record_initialize(
     void )
{
	libcerror_error_t *error                        = NULL;
	libfshfs_btree_node_record_t *btree_node_record = NULL;
	int result                                      = 0;
	...
	/* Test regular cases
	 */
	result = libfshfs_btree_node_record_initialize(
	          &btree_node_record,
	          &error );
	...
	result = libfshfs_btree_node_record_free(
	          &btree_node_record,
	          &error );
	...
	btree_node_record = (libfshfs_btree_node_record_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfshfs_btree_node_record_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfshfs_btree_node_record_initialize with memset failing
		 */
...
/* Tests the libfshfs_btree_node_record_free function
 * Returns 1 if successful or 0 if not
 */
int fshfs_test_btree_node_record_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FSHFS_TEST_RUN(
	 "libfshfs_btree_node_record_initialize",
	 fshfs_test_btree_node_record_initialize );

	FSHFS_TEST_RUN(
	 "libfshfs_btree_node_record_free",
	 fshfs_test_btree_node_record_free );
