...
#include "fshfs_test_libclocale.h"
#include "fshfs_test_libcsystem.h"
 ...
 */
int fshfs_test_volume_get_narrow_source(
     const system_character_t *source,
     char *narrow_string,
     size_t narrow_string_size,
     libcerror_error_t **error )
{
	static char *function     = "fshfs_test_volume_get_narrow_source";
 ...
 * Returns 1 if successful or -1 on error
 */
int fshfs_test_volume_get_wide_source(
     const system_character_t *source,
     wchar_t *wide_string,
     size_t wide_string_size,
     libcerror_error_t **error )
{
	static char *function   = "fshfs_test_volume_get_wide_source";
     ...
     void )
{
	...
	/* Test libfshfs_volume_initialize
	 */
...
/* Tests the libfshfs_volume_open functions
 * Returns 1 if successful or 0 if not
 */
     ...
     const system_character_t *source )
{
	...
	/* Initialize test
	 */
	result = fshfs_test_volume_get_narrow_source(
	          source,
	          narrow_source,
	          256,
	          &error );
...
/* Tests the libfshfs_volume_open_wide functions
 * Returns 1 if successful or 0 if not
 */
     ...
     const system_character_t *source )
{
	...
	/* Initialize test
	 */
	result = fshfs_test_volume_get_wide_source(
	          source,
	          wide_source,
	          256,
	          &error );
...
#endif
{
	...
	if( source != NULL )
	{
		...
		/* TODO add test for libfshfs_volume_open_file_io_handle */

#endif /* defined( LIBFSHFS_HAVE_BFIO ) */

		/* TODO add test for libfshfs_volume_close */
...
on_error:
	...
	if( volume != NULL )
	{
		fshfs_test_volume_close_source(
		 &volume,
		 NULL );
	}
	return( EXIT_FAILURE );
