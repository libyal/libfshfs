/*
 * Library btree_node_descriptor type test program
 *
...
#include "../libfshfs/libfshfs_btree_node_descriptor.h"

uint8_t fshfs_test_btree_node_descriptor_data1[ 14 ] = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x03, 0x00, 0x00 };

#if defined( __GNUC__ ) && !defined( LIBFSHFS_DLL_IMPORT )

/* Tests the libfshfs_btree_node_descriptor_initialize function
 * Returns 1 if successful or 0 if not
 */
int fshfs_test_btree_node_descriptor_initialize(
     void )
{
	libcerror_error_t *error                                = NULL;
	libfshfs_btree_node_descriptor_t *btree_node_descriptor = NULL;
	int result                                              = 0;
	...
	/* Test regular cases
	 */
	result = libfshfs_btree_node_descriptor_initialize(
	...
	result = libfshfs_btree_node_descriptor_free(
	          &btree_node_descriptor,
	          &error );
	...
	btree_node_descriptor = (libfshfs_btree_node_descriptor_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfshfs_btree_node_descriptor_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfshfs_btree_node_descriptor_initialize with memset failing
		 */
...
/* Tests the libfshfs_btree_node_descriptor_free function
 * Returns 1 if successful or 0 if not
 */
int fshfs_test_btree_node_descriptor_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libfshfs_btree_node_descriptor_read_data function
 * Returns 1 if successful or 0 if not
 */
int fshfs_test_btree_node_descriptor_read_data(
     void )
{
	libcerror_error_t *error                                = NULL;
	libfshfs_btree_node_descriptor_t *btree_node_descriptor = NULL;
	...
	/* Test regular cases
	 */
	result = libfshfs_btree_node_descriptor_read_data(
	          btree_node_descriptor,
	          fshfs_test_btree_node_descriptor_data1,
	          14,
	          &error );
...
#endif
{
	...
	FSHFS_TEST_RUN(
	 ...
	 fshfs_test_btree_node_descriptor_initialize );

	FSHFS_TEST_RUN(
	 "libfshfs_btree_node_descriptor_free",
	 fshfs_test_btree_node_descriptor_free );

	FSHFS_TEST_RUN(
	 "libfshfs_btree_node_descriptor_read_data",
	 fshfs_test_btree_node_descriptor_read_data );
